...
**Key Findings:**
- Hotels price location and product features correctly (R² = 0.71)
- Hotels systematically ignore occupancy signals (correlation r = 0.11)
- Demand is inelastic, meaning pricing power exists
- Annual revenue opportunity: €4.3M (conservative) to €20M (optimistic)
- Per-hotel impact: €19,549 median annual increase (range: €9 to €460k)
...
### 1.1 The Original Assignment

Amenitiz, a provider of hotel management tools, sought to develop **PriceAdvisor**, a price recommendation system to help hoteliers optimize their pricing strategies and maximize Revenue Per Available Room (RevPAR). The assignment was straightforward:

> Build a model that will be able to recommend optimal room prices to hoteliers for the given parameters.

The requirements included:
- Start with a baseline model better than random pricing
- Show evaluation and improvement of model performance
- Use metrics that test how well the model predicts optimal prices or maximizes revenue
- Derive additional features to demonstrate data sense and creativity
- Identify opportunities for using the model in the company
...
### 2.10 Reproducibility and Transparency

All data cleaning steps are:
- **Documented:** 31 rules with clear business logic
- **Versioned:** Cleaning configuration stored in code
- **Auditable:** Before/after metrics tracked
- **Reversible:** Original data preserved, cleaning is non-destructive

This transparency is essential for:
- Peer review and validation
- Regulatory compliance (if used for pricing decisions)
- Continuous improvement (updating rules as data evolves)
- Stakeholder trust (showing we didn't "cherry-pick" data)
