...
def plot_booking_counts_analysis(
    ...
    if prophet_results is not None:
        ...
        # Descriptive only - plot actual data
        ax1.plot(stats_df['agg_date'], stats_df['num_bookings'], 
                 linewidth=0.8, alpha=0.6, label='Actual', color='steelblue')
    ...
    if seasonal_patterns is not None:
        ...
        # Calculate from stats_df if seasonal_patterns not available
        stats_df['month'] = pd.to_datetime(stats_df['agg_date']).dt.month
        monthly_avg = stats_df.groupby('month')['num_bookings'].mean()
        month_names = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun',
                       'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
        ax2.bar(range(1, 13), [monthly_avg.get(m, 0) for m in range(1, 13)], 
    ...
    if seasonal_patterns is not None:
        ...
        # Calculate from stats_df if seasonal_patterns not available
        stats_df['quarter'] = pd.to_datetime(stats_df['agg_date']).dt.quarter
        quarterly_avg = stats_df.groupby('quarter')['num_bookings'].mean()
        ax3.bar(quarterly_avg.index, quarterly_avg.values, color='coral', alpha=0.7)
    ...
    if 'year' not in stats_df.columns:
        stats_df['year'] = pd.to_datetime(stats_df['agg_date']).dt.year
    if 'month' not in stats_df.columns:
        stats_df['month'] = pd.to_datetime(stats_df['agg_date']).dt.month
...
def print_booking_counts_summary(
    ...
    if prophet_results is not None:
        ...
        print(f"\n4. PROPHET MODEL FIT:")
        print("   (Prophet model not used - descriptive analysis only)")
    ...
    if seasonal_patterns is not None:
        ...
        if seasonal_patterns.get('weekend_avg') is not None:
        ...
        print(f"\n5. SEASONAL PATTERNS:")
        print("   (Seasonal patterns not analyzed - descriptive analysis only)")
...
def identify_underpricing_opportunities(
    ...
    # REFINED CRITERIA: Must also confirm that last-minute prices are ACTUALLY discounted
    # compared to advance prices (not just volume-based)
    underpriced = daily_stats[
        (daily_stats['occupancy_rate'] >= min_occupancy) &
        (daily_stats['last_minute_pct'] >= min_last_minute_pct) &
        # CRITICAL: Only flag if last-minute prices are LOWER than advance prices
        # This confirms an actual discount was given, not just high last-minute volume
        (daily_stats['avg_last_minute_price'] < daily_stats['avg_advance_price'])
    ].copy()
